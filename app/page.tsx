...
export default function Home() {
  ...
  const [completingScheduleId, setCompletingScheduleId] = useState<
    string | null
  >(null);
  const [pendingCompletion, setPendingCompletion] = useState<Schedule | null>(
    null,
  );
  ...
  const dayTransactions = useMemo(
    () => data?.transactions.filter((t) => t.date === selectedDate) ?? [],
    [data, selectedDate],
  );
  ...
  const monthTransactions =
    data?.transactions.filter((t) => t.date.startsWith(monthKey)) ?? [];
    ...
    data?.schedules
      .filter(
        (s) =>
          ...
          s.status !== "cancelled" &&
          !actualScheduleIds.has(s.id),
      )
      .reduce((sum, s) => sum + s.expectedCost, 0) ?? 0;
  ...
  const saveSchedule = async (event: FormEvent) => {
    ...
    if (!scheduleDraft?.title.trim() || !data || scheduleSaveLock.current)
      return;
    const selectedStartTime =
      completingScheduleId === scheduleDraft.id
        ? scheduleDraft.actualStartTime
        : scheduleDraft.startTime;
    const selectedEndTime =
      completingScheduleId === scheduleDraft.id
        ? scheduleDraft.actualEndTime
    ...
    const item = {
      ...scheduleDraft,
      id: scheduleDraft.id || newId(),
      userId: data.user.id,
      expectedCost: Number(scheduleDraft.expectedCost) || 0,
    };
    try {
      if (completingScheduleId === item.id) {
        const actual = data.transactions.find(
          (transaction) =>
            ...
            transaction.scheduleId === item.id,
        );
      ...
      if (scheduleDraft.id && editScheduleSeries && item.seriesId) {
        const occurrences = data.schedules
          .filter(
            (schedule) =>
              schedule.seriesId === item.seriesId && schedule.date >= item.date,
          )
          .map((schedule) => ({
            ...schedule,
            title: item.title,
        ...
        const replacements = new Map(
          occurrences.map((schedule) => [schedule.id, schedule]),
        );
        setData(
          (prev) =>
            prev && {
              ...
              schedules: prev.schedules.map(
                (schedule) => replacements.get(schedule.id) ?? schedule,
              ),
            },
        );
        setScheduleDraft(null);
      ...
      if (
        ...
        setData(
          (prev) =>
            prev && { ...prev, schedules: [...prev.schedules, ...occurrences] },
        );
        setScheduleDraft(null);
      ...
      setData(
        (prev) =>
          prev && {
            ...prev,
            schedules: prev.schedules.some((s) => s.id === completedItem.id)
              ? prev.schedules.map((s) =>
                  s.id === completedItem.id ? completedItem : s,
                )
              : [...prev.schedules, completedItem],
          },
      );
      setScheduleDraft(null);
        ...
        (prev) =>
          prev && {
            ...
            transactions: prev.transactions.map((t) =>
              t.scheduleId === id ? { ...t, scheduleId: undefined } : t,
            ),
            dailyRecords: prev.dailyRecords.map((r) =>
              r.scheduleId === id ? { ...r, scheduleId: undefined } : r,
            ),
          },
      );
      setMenuId(null);
      setToast("일정을 삭제했어요. 실제 거래와 하루 기록은 유지돼요");
      return true;
    } catch (error) {
      notifyError(error);
      return false;
    ...
    try {
      ...
      setData(
        (prev) =>
          prev && {
            ...prev,
            schedules: prev.schedules.map((s) => (s.id === item.id ? item : s)),
          },
      );
      setMenuId(null);
    } catch (error) {
  ...
  const saveTransaction = async (event: FormEvent) => {
    ...
    const sameScheduleTransaction =
      txDraft.source === "schedule_actual" && txDraft.scheduleId
        ? data.transactions.find(
            (transaction) =>
              transaction.scheduleId === txDraft.scheduleId &&
              transaction.id !== txDraft.id,
          )
        : undefined;
    const item = {
      ...txDraft,
      id: txDraft.id || sameScheduleTransaction?.id || newId(),
      userId: data.user.id,
      amount: Number(txDraft.amount),
    };
    const scheduleUpdate: Schedule | undefined =
      pendingCompletion &&
      item.source === "schedule_actual" &&
      pendingCompletion.id === item.scheduleId
        ? {
            ...pendingCompletion,
            status: "done" as const,
            completedAt: new Date().toISOString(),
    ...
    try {
      if (txDraft.id && editTransactionSeries && item.seriesId) {
        const occurrences = data.transactions
          .filter(
            (transaction) =>
              transaction.seriesId === item.seriesId &&
        ...
        const replacements = new Map(
          occurrences.map((transaction) => [transaction.id, transaction]),
        );
        setData(
          (prev) =>
            prev && {
      ...
      if (
        ...
        setData(
          (prev) =>
            prev && {
              ...prev,
              transactions: [...prev.transactions, ...occurrences],
            },
        );
        setTxDraft(null);
      ...
      await Promise.all([
        ...
        scheduleUpdate ? upsertSchedule(scheduleUpdate) : Promise.resolve(),
      ]);
      setData(
        (prev) =>
          prev && {
            ...
            schedules: scheduleUpdate
              ? prev.schedules.map((schedule) =>
                  schedule.id === scheduleUpdate.id ? scheduleUpdate : schedule,
                )
              : prev.schedules,
            transactions: prev.transactions.some(
    ...
    try {
      ...
      setData(
        (prev) =>
          prev && {
            ...prev,
            diaries: prev.diaries.some((d) => d.date === item.date)
              ? prev.diaries.map((d) => (d.date === item.date ? item : d))
              : [...prev.diaries, item],
          },
      );
  ...
  const saveRecord = async (event: FormEvent) => {
    event.preventDefault();
    if (!recordDraft?.title.trim() || !recordDraft.content.trim() || !data)
      return;
    const item = {
      ...recordDraft,
      id: recordDraft.id || newId(),
      userId: data.user.id,
    };
    try {
      ...
      setData(
        (prev) =>
          prev && {
            ...prev,
            dailyRecords: prev.dailyRecords.some((r) => r.id === item.id)
              ? prev.dailyRecords.map((r) => (r.id === item.id ? item : r))
              : [...prev.dailyRecords, item],
          },
      );
      setRecordDraft(null);
      setToast("하루 기록을 저장했어요");
    } catch (error) {
      notifyError(error);
    }
  };
  ...
  const editActualExpense = (schedule: Schedule) => {
    ...
    setTxDraft(
      actual
        ...
        : {
            ...blankTransaction(
              schedule.actualDate || schedule.date,
              schedule.userId,
            ),
            title: schedule.title,
            scheduleId: schedule.id,
            source: "schedule_actual",
            amount: schedule.expectedCost,
          },
    );
  };
  ...
  const deleteScheduleFromModal = async () => {
    if (
      !scheduleDraft?.id ||
      !window.confirm(
        `“${scheduleDraft.title}” 일정을 삭제할까요?\n연결된 실제 거래와 하루 기록은 유지됩니다.`,
      )
    )
      return;
  ...
  if (loadError)
    return (
      <main className="loading error-state">
        <b>데이터 연결이 잠시 지연되고 있어요</b>
        <span>{loadError}</span>
        <button
          className="primary"
          onClick={() => {
            setLoadError("");
            setDataLoadRevision((revision) => revision + 1);
          }}
        >
          다시 연결
        </button>
    ...
    <div className={`app-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom">
          ...
          />
          <button
            className={`profile profile-button ${view === "profile" ? "active" : ""}`}
      ...
      <header className="mobile-topbar">
        <button
          ...
          onClick={() => changeMobileView("today")}
          aria-label="작은다음 오늘 화면으로 이동"
        >
          <BrandLogo />
        </button>
        <button
          className={`mobile-profile ${view === "profile" ? "active" : ""}`}
          aria-label="프로필 열기"
          onClick={() => changeMobileView("profile")}
        >
          <span>{data.user.name.slice(0, 1).toUpperCase()}</span>
        </button>
      </header>
      ...
      <main className={`content view-${view}`}>
        ...
        {view === "calendar" && (
          <CalendarView
            ...
            onAdd={(date, startTime, endTime) =>
              setScheduleDraft({
                ...blankSchedule(date, data.user.id),
        ...
        {view === "records" && (
          <RecordsView
            ...
            onEdit={setRecordDraft}
            onDelete={deleteRecord}
          />
        )}
        {view === "diary" && (
          <DiaryView
            key={`${selectedDate}-${dayDiary?.id ?? "new"}`}
            date={selectedDate}
            onDate={changeDate}
            diary={dayDiary}
            schedules={daySchedules}
            ...
            onAddUnplanned={() =>
              setScheduleDraft({
                ...blankSchedule(selectedDate, data.user.id),
                status: "done",
              })
            }
          />
        ...
        {view === "settings" && (
          <SettingsView data={data} setData={setData} onToast={setToast} />
        )}
      </main>
      ...
      <div className="mobile-nav">
        ...
        <NavButton
          ...
          label="캘린더"
          active={view === "calendar"}
          onClick={() => changeMobileView("calendar")}
        />
        <button
        ...
        <NavButton
          ...
          label="가계부"
          active={view === "ledger"}
          onClick={() => changeMobileView("ledger")}
        />
        <NavButton
          icon="•••"
          label="더보기"
          active={
            mobileMenuOpen ||
            ["stats", "records", "diary", "settings"].includes(view)
          }
      ...
      {mobileMenuOpen && (
        <dialog open className="mobile-more-backdrop" aria-label="더보기 메뉴">
          <section className="mobile-more-sheet">
            ...
            <button onClick={() => changeMobileView("records")}>
              <span>✎</span>
              <b>하루 기록</b>
              <small>작은 순간을 자유롭게 기록</small>
            </button>
            <button onClick={() => changeMobileView("diary")}>
              <span>♡</span>
              <b>감정 일기</b>
              <small>하루의 감정과 강도 돌아보기</small>
            </button>
            <button onClick={() => changeMobileView("stats")}>
              <span>▥</span>
              <b>통계</b>
              <small>일정, 지출, 감정 흐름 확인</small>
            </button>
            <button onClick={() => changeMobileView("settings")}>
              <span>⚙</span>
              <b>설정</b>
              <small>계정과 데이터 관리</small>
            </button>
          </section>
        </dialog>
      )}
      ...
      {scheduleDraft && (
        <ScheduleModal
          ...
          completionMode={completingScheduleId === scheduleDraft.id}
          hasActualExpense={data.transactions.some(
            (transaction) =>
              transaction.source === "schedule_actual" &&
              transaction.scheduleId === scheduleDraft.id,
          )}
          editSeries={editScheduleSeries}
          setEditSeries={setEditScheduleSeries}
          saving={savingSchedule}
          onClose={closeScheduleModal}
          onDelete={
            scheduleDraft.id && completingScheduleId !== scheduleDraft.id
      ...
      {txDraft && (
        <TransactionModal
          draft={txDraft}
          setDraft={setTxDraft}
          schedules={data.schedules.filter((s) => s.date === txDraft.date)}
          lockedScheduleId={actualScheduleId}
          editSeries={editTransactionSeries}
          setEditSeries={setEditTransactionSeries}
      ...
      {recordDraft && (
        <RecordModal
          ...
          schedules={data.schedules.filter((s) => s.date === recordDraft.date)}
