...
export const statusMap: Record<ScheduleStatus, string> = { planned: "예정", done: "완료", partial: "일부 완료", cancelled: "취소" };
export const expenseCategories = ["식비", "교통", "생활", "주거", "문화", "건강", "교육", "쇼핑", "기타 지출"];
export const incomeCategories = ["급여", "용돈", "사업 소득", "상여", "이자", "배당", "환급", "중고 판매", "기타 수입"];
export const koreanHolidays = holidayPresets as unknown as Record<string, Record<string, readonly string[]>>;
...
export const displayDate = (date: string, withYear = false) => new Intl.DateTimeFormat("ko-KR", withYear
  ? { year: "numeric", month: "long", day: "numeric", weekday: "long" }
  : { month: "long", day: "numeric", weekday: "short" }).format(new Date(`${date}T00:00:00`));
...
export const strongestEmotion = (diary?: EmotionDiary) => {
  ...
  const entry = first ? rest.reduce((strongest, current) => current.intensity > strongest.intensity ? current : strongest, first) : undefined;
  return entry ? emotions.find((emotion) => emotion.value === entry.emotion) : undefined;
};