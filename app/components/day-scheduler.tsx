...
export function DayScheduler({
  ...
  return (
    <section className="day-scheduler">
      <div className="scheduler-intro">
        <div>
          <p>DAY PLANNER</p>
          <h2>{displayDate(date, true)}</h2>
        </div>
        <button className="primary" onClick={() => onAdd(date)}>
          ＋ 일정 추가
        </button>
      </div>
      <div className="scheduler-summary" aria-label="선택한 날짜의 일정 요약">
        <span>
          <small>예정된 일정</small>
          <b>{schedules.length}개</b>
        </span>
        <span>
          <small>계획한 시간</small>
          <b>{durationLabel(plannedMinutes)}</b>
        </span>
        <span>
          <small>완료</small>
          <b>{completed}개</b>
              ...
              <button
                ...
                onClick={() =>
                  onAdd(
                    date,
                    timeAt(hour),
                    hour === 23 ? "23:50" : timeAt(hour + 1),
                  )
                }
                aria-label={`${timeAt(hour)}에 일정 추가`}
              >
                <time>{timeAt(hour)}</time>
                <span>이 시간에 일정 추가</span>
              </button>
            ))}
          </div>
          <div className="scheduler-events">
            {positioned.map(({ schedule, start, end, lane, lanes }) => {
              ...
              return (
                <button
                  type="button"
                  className={`scheduler-event ${schedule.status}`}
                  style={style}
                  key={schedule.id}
