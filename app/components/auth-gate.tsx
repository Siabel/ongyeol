...
function authErrorMessage(message: string) {
  if (/user already registered/i.test(message))
    return "이미 가입되어 있는 이메일입니다. 로그인하거나 비밀번호 찾기를 이용해 주세요.";
  if (/invalid login credentials/i.test(message))
    return "이메일 또는 비밀번호가 올바르지 않아요.";
  if (/email not confirmed/i.test(message))
    return "이메일 인증이 완료되지 않았어요. 받은 메일의 인증 링크를 확인해 주세요.";
  if (/password.*at least|weak password/i.test(message))
    return "비밀번호가 보안 기준을 충족하지 않아요. 6자 이상으로 입력해 주세요.";
  if (/rate limit|too many requests/i.test(message))
    return "요청이 너무 많아요. 잠시 후 다시 시도해 주세요.";
  if (/email address not authorized/i.test(message))
    return "현재 메일 발송 설정에서 허용되지 않은 이메일이에요. 관리자에게 문의해 주세요.";
  if (/failed to fetch|fetch failed|network/i.test(message))
    return "로그인 서버에 연결할 수 없어요. 잠시 후 다시 시도하거나 Supabase 연결 설정을 확인해 주세요.";
  return message;
}
...
function validate(
  ...
  if (mode === "signup") {
    if (!name) errors.name = "닉네임을 입력해 주세요.";
      ...
      errors.name = "닉네임은 30자 이내로 입력해 주세요.";
    else if (invalidNamePattern.test(name))
      errors.name = "닉네임에 사용할 수 없는 문자가 포함되어 있어요.";
  }
      ...
      <section className="auth-stage">
        <aside className="auth-story">
          ...
          <div>
            ...
            <span>일정과 감정, 소비와 작은 순간을 차곡차곡.</span>
          </div>
        </aside>
        <section className="auth-card setup-card">
          <div className="auth-card-mark">설정</div>
          <p className="auth-kicker">SUPABASE SETUP</p>
          <h1>데이터 저장소를 연결해 주세요</h1>
          <p>
            회원가입과 사용자별 기록 저장 기능은 준비되어 있습니다. 프로젝트
            루트의 <code>.env.local</code>에 Supabase 프로젝트 URL과 Publishable
            Key를 입력하면 로그인 화면이 열립니다.
          </p>
          ...
          <small>
            키를 입력한 뒤 개발 서버를 다시 시작해 주세요. 비밀키인 service_role
            키는 넣지 마세요.
          </small>
        </section>
      </section>
...
export function AuthGate() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (mode === "signup" && blockedSignupEmail === normalizedEmail) {
      setNotice({
        kind: "error",
        text: "이미 가입되어 있는 이메일입니다. 로그인하거나 비밀번호 찾기를 이용해 주세요.",
      });
      return;
    }
    ...
    try {
      if (mode === "recovery") {
        ...
        if (error)
          setNotice({ kind: "error", text: authErrorMessage(error.message) });
        else
          setNotice({
            kind: "success",
            text: "비밀번호 변경 메일을 요청했어요. 가입된 이메일이라면 잠시 후 메일이 도착합니다.",
          });
        return;
      }
      ...
      const duplicateSignup =
        mode === "signup" &&
        (Boolean(
          result.error && /user already registered/i.test(result.error.message),
        ) ||
          Boolean(
            !result.error &&
            result.data.user &&
      ...
      if (duplicateSignup) {
        ...
        setNotice({
          kind: "error",
          text: authErrorMessage(result.error.message),
        });
      } else if (mode === "signup" && !result.data.session) {
        setNotice({
          kind: "success",
          text: "인증 메일을 보냈습니다. 이메일 인증 후 로그인해 주세요.",
        });
      }
    } catch (error) {
      ...
      setNotice({
        kind: "error",
        text: "로그인 서버에 연결할 수 없어요. 잠시 후 다시 시도해 주세요.",
      });
    } finally {
      setBusy(false);
    }
    ...
    <main className="auth-page">
      <section className="auth-stage">
        <aside className="auth-story">
          ...
          <small>아주 작은 기록도 다음으로 가는 힘이 됩니다.</small>
        </aside>
        <section className="auth-card">
          <div className="auth-card-mark">다</div>
          <p className="auth-kicker">YOUR SMALL NEXT</p>
          <h1>{title}</h1>
          <p>{description}</p>
          <form onSubmit={submit} noValidate>
            ...
            <button
              ...
              disabled={
                busy ||
                (mode === "signup" &&
                  blockedSignupEmail === email.trim().toLowerCase())
              }
            >
              {busy
                ? "처리 중…"
          ...
          >
            {mode === "login" ? "처음인가요? 회원가입" : "로그인으로 돌아가기"}
          </button>
        </section>
      </section>
    </main>
