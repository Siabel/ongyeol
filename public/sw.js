const CACHE_VERSION = "jakda-shell-v1";
const CORE_ASSETS = [
  "/",
...
self.addEventListener("activate", (event) => {
  event.waitUntil(
    caches.keys()
      .then((keys) => Promise.all(keys.filter((key) => (key.startsWith("jakda-") || key.startsWith("ongyeol-")) && key !== CACHE_VERSION).map((key) => caches.delete(key))))
