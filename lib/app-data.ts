...
function diaryEmotions(row: Record<string, unknown>): EmotionEntry[] {
  ...
  return [
    {
      emotion: String(row.emotion ?? "calm"),
      intensity: Number(row.intensity) || 3,
    },
  ];
}

export async function loadAppData(user: User): Promise<AppData> {
  const client = db();
  const loadTables = () =>
    Promise.all([
  ...
  if (result.some(({ error }) => jwtFutureError(error))) {
    throw new Error(
      "로그인 정보의 시간 동기화가 지연되고 있어요. 잠시 후 다시 연결해 주세요.",
    );
  }
  [schedules, transactions, diaries, records].forEach(({ error }) =>
    throwIfError(error),
  );
  ...
  const data = emptyAppData({
    ...
    email: user.email ?? "",
    name: String(
      user.user_metadata?.display_name ?? user.email?.split("@")[0] ?? "나",
    ),
  ...
  data.schedules = (schedules.data ?? []).map((r) => ({
    ...
    actualStartTime: r.actual_start_time?.slice(0, 5) ?? "",
    actualEndTime: r.actual_end_time?.slice(0, 5) ?? "",
    completedAt: r.completed_at ?? null,
    place: r.place_name ?? "",
    address: r.address ?? "",
    people: r.people ?? "",
    expectedCost: Number(r.expected_cost),
    status: r.status,
    memo: r.memo ?? "",
  ...
  data.transactions = (transactions.data ?? []).map((r) => ({
    ...
    title: r.title,
    memo: r.memo ?? "",
    source: r.source ?? (r.schedule_id ? "schedule_actual" : "manual"),
    fixed: r.is_fixed,
    repeatFrequency: r.repeat_frequency ?? null,
...
export async function upsertSchedule(item: Schedule) {
  const { error } = await db()
    ...
    .upsert({
      ...
      repeat_frequency: item.isRecurring ? item.repeatFrequency : null,
      repeat_end_date: item.isRecurring ? item.repeatEndDate : null,
...
export async function upsertTransaction(item: Transaction) {
  const { error } = await db()
    ...
    .upsert({
      ...
      source: item.source,
      is_fixed: item.fixed,
      is_repeat: item.fixed,
      repeat_frequency: item.fixed ? item.repeatFrequency : null,
      repeat_month:
        item.fixed && item.repeatFrequency === "yearly"
          ? item.repeatMonth
          : null,
      repeat_day:
        item.fixed && item.repeatFrequency !== "daily" ? item.repeatDay : null,
      repeat_end_date: item.fixed ? item.repeatEndDate : null,
      series_id: item.seriesId,
